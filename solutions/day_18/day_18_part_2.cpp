...
public:
    ...
    DigCommand(std::string_view text_command){
        auto code = split(text_command, ' ')[2];
        code = code.substr(2, code.length()-3);
        length = hex_to_int(code.substr(0,code.length()-1));
        switch (code.back())
        {
        case '0':
            dir = Coord::East;
...
public:
    ...
    void load_command(DigCommand command){
        ...
        current_pos = current_pos + current_dir*command.length;
    ...
    long long int trench_volume(){
        ...
        return  static_cast<long long int>(trench_volume);
    }
};
...
int main(int argc, [[maybe_unused]] char *argv[]) {
    Solution solution = Solution(18, 2, solve);
    if (argc > 1){
