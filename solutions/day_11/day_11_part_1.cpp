...
public:
    ...
    int sum_of_distances_between_galaxies(){
        int sum = 0;
        for (const Coord& galaxy_1 : galaxies)
            for (const Coord& galaxy_2 : galaxies)
                sum += expanded_distance(galaxy_1, galaxy_2);

        //Each distance is calculated twice
        return sum/2;
    }
private:
    int expanded_distance(Coord galaxy_1, Coord galaxy_2){
        ...
        return standard_distance + vertical_expansion + horizontal_expansion;
    }
...
int main(int argc, [[maybe_unused]] char *argv[]) {
    Solution solution = Solution(11, 1, solve);
    if (argc > 1){
